...
void SimpleAnomalyDetector::detectTotxtFile(const TimeSeries& ts, const char* txtFileName) {
	...
	myfile << "Line\n";
	vector<AnomalyReport> v;
	for_each(cf.begin(), cf.end(), [&myfile,&v, &ts, this](correlatedFeatures c) {
		...
		//myfile << to_string(c.lin_reg.a) + "," + to_string(c.lin_reg.b) + "\n";
		...
		for (float i = minX; i < maxX; i=i + 0.25)
		{
			myfile << to_string(i) + ",";
		}
		myfile << to_string(maxX) + "\n";

		for (float i = minX; i < maxX; i = i + 0.25)
		{
			float yPoint = ((c.lin_reg.a) * i) + (c.lin_reg.b);
			myfile << to_string(yPoint) + ",";
		}
		float yPoint = ((c.lin_reg.a) * maxX) + (c.lin_reg.b);
		myfile << to_string(yPoint) + "\n";
